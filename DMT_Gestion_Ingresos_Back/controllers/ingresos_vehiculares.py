...
gestor = GestorIngresosDB("vehicular")
...
async def crear_ingreso_vehicular(datos: CrearIngresoVehicularRequest):
    try:
        ...
        return {"exito": True, "mensaje": "Ingreso vehicular registrado exitosamente", **resultado}
...
async def registrar_salida_vehicular(ticket: str, datos: RegistrarSalidaRequest):
    try:
        ...
        logger.error(f"Error en registrar_salida_vehicular: {e}")
        raise HTTPException(status_code=500, detail=f"Error interno: {e}")
